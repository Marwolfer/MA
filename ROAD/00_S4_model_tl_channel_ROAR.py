...
def build_model_for_finetuning(cfg, input_shape_st, device, save_path, epoch, explanation_func_name=None, k=None):
    ...
    if cfg.training.finetune_entire_model:
        ...
        if explanation_func_name is None:
            ...
            full_checkpoint_path = os.path.join(save_path, f"checkpoint_full_epoch_1000_explanation_func_{explanation_func_name}_k_{k}.pt")
            full_checkpoint = torch.load(full_checkpoint_path)
            model.load_state_dict(full_checkpoint['model_state_dict'])
...
def pretrain_or_finetune_model(model, ema_model, train_loader, cfg, device, labels_scaler, mean_mep, accelerator, window_idx, is_pretraining=True, explanation_func_name=None, k=None):
    ...
    if explanation_func_name is not None:
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        if is_pretraining:
            PATH = os.path.join(path, f"model_checkpoint_pretrain_subject_index_{subject_index}_window_idx_{window_idx}_explanation_func_name_{explanation_func_name}_k_{k}.pt")
        else:
            PATH =  os.path.join(path, f"model_checkpoint_pretrain_subject_index_{subject_index}_window_idx_{window_idx}_explanation_func_name_{explanation_func_name}_k_{k}.pt")
...
def continuous_finetuning(model, ema_model, data_window, optimizer, scheduler, cfg, device, labels_scaler, mean_mep, accelerator, start_index, num_epochs=1, explanation_func_name=None, k=None, rep=None):
    ...
    with torch.no_grad():
        if explanation_func_name is not None:
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            PATH = os.path.join(path, f"model_checkpoint_finetune_subject_index_{subject_index}_start_idx_{start_index}_rep_{rep}_explanation_func_name_{explanation_func_name}_k_{k}.pth")
            torch.save(model.state_dict(), PATH)
...
for subject_index in cfg.dataset.test_subject_indices:
    ...
    # maybe it makes sense to directly do this directly here instead
...
# for all of the following trials, explanations need to be generated
        ...
        avg_loss, metrics, calibration_metrics = continuous_finetuning(model, ema_model, data_window, optimizer, scheduler, cfg, device, labels_scaler, mean_mep, accelerator, num_epochs=1)
...
"""
        for rep in range(Reps):
            ...
            for explanation_func, explanation_func_name in zip(explanation_functions, explanation_function_names):
                ...
                for k in ks:
                    save_path = os.path.join("/home/marco/Documents/GitHub/tms_eeg_decoding/exp", explanation_func_name, str(k))
                    ...
                    all_epochs, all_labels_raw, fixed_median, fixed_q1, fixed_q3, labels_scaler, mean_mep, ch_names = load_eeg_data(cfg, explanation_func_name=explanation_func_name, k=k)
                    model, ema_model = pretrain_model(model, ema_model, all_epochs, all_labels_raw, fixed_median, fixed_q1, fixed_q3, cfg, device, labels_scaler, mean_mep, accelerator)
                    ...
                    metrics_filename = os.path.join(eval_save_path, f'all_metrics_subject_{subject_index}_rep_{rep}_explantion_func_{explanation_func_name}_k_{k}.npz')
