...
def build_model_for_finetuning(cfg, input_shape_st, device, save_path, epoch, freq_band=None):
    ...
    if cfg.training.finetune_entire_model:
        ...
        if freq_band is not None:
            full_checkpoint_path = os.path.join(save_path, f"checkpoint_full_epoch_{epoch}_freq_band_{freq_band}.pt")
...
def pretrain_or_finetune_model(model, ema_model, train_loader, cfg, device, labels_scaler, mean_mep, accelerator, window_idx, is_pretraining=True):
    ...
    path = os.path.join(path, "model_checkpoints", f"subject_{subject_index}")
    if not os.path.exists(path):
        os.makedirs(path)
    if is_pretraining:
        PATH = os.path.join(path, f"model_checkpoint_pretrain_subject_index_{subject_index}_window_idx_{window_idx}_rep_{rep}_freq_band_{freq_band}.pth")
    else:
        PATH = os.path.join(path, f"model_checkpoint_finetune_subject_index_{subject_index}_window_idx_{window_idx}_rep_{rep}_freq_band_{freq_band}.pth")
...
def continuous_finetuning(model, ema_model, data_window, optimizer, scheduler, cfg, device, labels_scaler, mean_mep, accelerator, start_index, num_epochs=1):
    model.train()
    ...
    with torch.no_grad():
        path = cfg.dataset.data_directory
        path = os.path.join(path, "model_checkpoints",f"subject_idx_{str(subject_index)}")
        if not os.path.exists(path):
            os.makedirs(path)
        PATH = os.path.join(path, f"model_checkpoint_finetune_subject_index_{subject_index}_start_idx_{start_index}_rep_{rep}_freq_band_{freq_band}.pth")
        torch.save(model.state_dict(), PATH)
...
for subject_index in cfg.dataset.test_subject_indices:
        ...
        for rep in range(Reps):
            ...
            for freq_band, freq_range in freq_bands.items():
                ...
                save_path_model_checkpoint = os.path.join(save_path, "S4EEGNet_pretrain_checkpoints")
                epoch = 1000
                model = build_model_for_finetuning(cfg, input_shape_st, device, save_path_model_checkpoint, epoch, freq_band=freq_band)
                ...
                all_epochs, all_labels_raw, fixed_median, fixed_q1, fixed_q3, labels_scaler, mean_mep, ch_names = load_eeg_data(cfg, freq_band=freq_band)
                ...
                for i in range(len(trial_indices)):
                    ...
                    'continuous_finetune_avg_loss': continuous_finetune_losses[i],
                    'continuous_finetune_rmse_standardized': continuous_finetune_rmse_standardized[i],
                    'continuous_finetune_rmse_original': continuous_finetune_rmse_original[i],
                    'continuous_finetune_relative_rmse': continuous_finetune_relative_rmse[i],
                    'continuous_finetune_uce': continuous_finetune_uce[i],
                    'current_learning_rate': learning_rates[i],
                    'rolling_rmse_standardized': rolling_rmse_standardized[i],
                    'rolling_rmse_original': rolling_rmse_original[i],
                    'rolling_rmse_relative': rolling_rmse_relative[i],
                    'rolling_binary_acc': rolling_binary_acc[i],
                    'fixed_binary_acc': fixed_binary_acc[i],
                    'rolling_3class_acc': rolling_3class_acc[i],
                    'rolling_uce': rolling_uce[i],
                    'regular_loss': regular_losses[i],
                    'ema_loss': ema_losses[i],
                ...
                metrics_filename = os.path.join(eval_save_path, f'all_metrics_subject_{subject_index}_rep_{rep}_freq_band_{freq_band}.npz')
