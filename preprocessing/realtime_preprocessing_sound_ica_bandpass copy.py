...
load_directory = r'/home/marco/Documents/GitHub/tms_eeg_decoding/data/frequency_bandstop'

# Define directories
#load_directory = r'C:\Users\Lisa Haxel\Documents\CEBRA\data_raw'
save_directory = r'/home/marco/Documents/GitHub/tms_eeg_decoding/data/frequency_bandstop/bandpass'
mep_directory = r'/home/marco/Documents/GitHub/tms_eeg_decoding/data/frequency_bandstop/bandpass'
...
#subject_indices = [1,2,3,4,7,9,11,13,14,18,22,24,25,26,27,29,31,33,34,35,39,41,42,43,45,46,47,48,51,52,53,55,56,57,59,60,62,63,65,66,67,69,70,71,72,73,74,76,79,80,83,86,88,92,100,102,103,104,106,107,108,109,110,111]
...
subject_files = [f'subject_{i}_mne_py.set' for i in subject_indices]
...
for freq_band, freq_range in frequency_bands.items():
    ...
    for subject_file in subject_files:
        ...
        np.save(os.path.join(save_directory, f"sub-{subject_number}_bandpass_{freq_band}_filtered_events.npy"), filtered_events)


        # save filtered_events as a txt file
        np.savetxt(os.path.join(save_directory, f"sub-{subject_number}_bandpass_{freq_band}_filtered_events.txt"), filtered_events, fmt='%d')	

        # load filtered_events
        filtered_events = np.load(os.path.join(save_directory, f"sub-{subject_number}_bandpass_{freq_band}_filtered_events.npy"))
        ...
        class ICAPreprocessor:
            def __init__(self, num_channels, n_components=25, tmin=0,  raw_info=None, eeg_picks=None, montage=None, bandpass_freq=None):
                ...
                if bandpass_freq is not None:
                    self.bandpass_freqs = bandpass_freq
                    self.mne_filter = MNEFilter(f_ranges=[self.bandpass_freqs], sfreq=self.resample_freq_hz, filter_length="1000ms", verbose=False)

                else:
                    self.mne_filter = MNEFilter(f_ranges=[(2, 45)], sfreq=self.resample_freq_hz, filter_length="1000ms", verbose=False)
        ...
        class SOUNDPreprocessor:
            def __init__(self, num_of_eeg_channels, sfreq, resample_freq_hz, leadfield_matrix, raw_info, eeg_picks, montage=None, ica_preprocessor=None, bandpass_freq=None):
                ...
                if bandpass_freq is not None:
                    ...
                    self.mne_filter = MNEFilter(f_ranges=[(2, 45)], sfreq=resample_freq_hz, filter_length="1000ms", verbose=False)
                # Initialize the Resampler and MNEFilter
        ...
        save_path = os.path.join(save_directory, f'subject_{subject_number}_bandpass_{freq_band}_mep_py_rt.csv')
        ...
        save_path = os.path.join(save_directory, f'subject_{subject_number}_bandpass_{freq_band}_eeg_preprocessed_py.fif')
